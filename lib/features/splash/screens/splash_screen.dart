...
class SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _handleDefaultRouting() async {
    if (Get.find<AuthController>().isLoggedIn()) {
      // Login token is already in SharedPreferences; FCM token refresh must
      // never block the auto-login flow on iOS release builds. Wrap in try/catch.
      try {
        await Get.find<AuthController>().updateToken();
      } catch (e) {
        if (kDebugMode) {
          print('---- updateToken() failed during splash auto-login (non-fatal): $e');
        }
      }
