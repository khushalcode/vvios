...
class AuthService implements AuthServiceInterface {
  ...
  /// Wraps [updateToken] in a try/catch so that any Firebase / FCM / APNs
  /// failure on iOS (e.g. mismatched GoogleService-Info.plist bundle id,
  /// missing APNs auth key, denied notification permission, or a null FCM
  /// token in release builds) does NOT break the login flow. The token is
  /// already saved before this runs, so the user is logged in regardless.
  /// The FCM token upload will be retried on the next app launch via
  /// [SplashScreen._handleDefaultRouting] -> updateToken.
  Future<void> _safeUpdateToken() async {
    try {
      await updateToken();
    } catch (e, stack) {
      if (kDebugMode) {
        print('---- updateToken() failed (non-fatal, login will continue): $e');
        print(stack);
      }
    }
  }
